...
def request_packet(version=NTP_CURRENT_VERSION):
    current_time = time()
    return SNTPPacket(version=version, transmit=utc_to_ntp_bytes(current_time))
...
def packet_from_binary(data):
    ...
    return SNTPPacket(leap, version, mode, stratum, poll, precision,
